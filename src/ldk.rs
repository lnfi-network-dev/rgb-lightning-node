...
const WEBHOOK_SUBSCRIPTIONS_KEY: &str = "webhook_subscriptions";
...
impl UnlockedAppState {
    ...
    fn save_webhook_subscriptions(&self, webhooks: MutexGuard<WebhookStorage>) {
        self.kv_store
            .write("", "", WEBHOOK_SUBSCRIPTIONS_KEY, webhooks.encode())
            .unwrap();
    }
...
pub(crate) async fn start_ldk(
    ...
    // Read webhook subscriptions from KVStore
    let webhook_subscriptions = Arc::new(Mutex::new({
        match kv_store.read("", "", WEBHOOK_SUBSCRIPTIONS_KEY) {
            Ok(bytes) => {
                WebhookStorage::read(&mut &bytes[..]).unwrap_or_else(|_| WebhookStorage {
                    subscriptions: Vec::new(),
                })
            }
            Err(e) if e.kind() == io::ErrorKind::NotFound => WebhookStorage {
                subscriptions: Vec::new(),
            },
            Err(e) => panic!("Failed to read webhook subscriptions from KVStore: {e}"),
        }
    }));
